...
# Required.
@click.option('--outdir',       help='Where to save the results', metavar='DIR',                default='/home/work/KimJungsoo/backup/StyleGAN3/Memristor-New-offline/')
@click.option('--cfg',          help='Base configuration',                                      type=click.Choice(['stylegan3-t', 'stylegan3-r', 'stylegan2']), default='stylegan3-r')
@click.option('--data',         help='Training data', metavar='[ZIP|DIR]',                      type=str, default='/home/work/KimJungsoo/dataset/Ocular/ND/original/A')
@click.option('--gpus',         help='Number of GPUs to use', metavar='INT',                    type=click.IntRange(min=1), default=1)
@click.option('--batch',        help='Total batch size', metavar='INT',                         type=click.IntRange(min=1), default=4)
@click.option('--gamma',        help='R1 regularization weight', metavar='FLOAT',               type=click.FloatRange(min=0), default=6.6)
...
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',            type=parse_comma_separated_list, default=['fid1k', 'kid1k'], show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name=None, z_dim=100, w_dim=256, mapping_kwargs=dnnlib.EasyDict())
